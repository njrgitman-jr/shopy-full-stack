...
export async function registerUserController(request, response) {
  try {
    ...
    const verifyEmail = await sendEmail({
      sendTo: email,
      subject: "Verify email from Shopy",
...
export async function updateUserDetails(request, response) {
  try {
    const userId = request.userId; //auth middleware..only loggedin user will be update
    ...
    //this can be destructure in form  const { data: responseData } = response; to get each property as in profile.jsx when updatedetails
    return response.json({
...
export async function forgotPasswordController(request, response) {
  try {
    ...
    await sendEmail({
      sendTo: email,
      subject: "Forgot password from Shopyit",
...
export async function verifyForgotPasswordOtp(request, response) {
  try {
    ...
    const user = await UserModel.findOne({ email }); //get the user id with the help of email will get user array details info
...
//get login user details #2 5:5:00
export async function userDetails(request, response) {
  try {
    ...
    //select with exclude fields password and refresh_token by adding prefix (-)
...
export async function getLoginHistory(request, response) {
  try {
    const userId = request.userId; // from auth middleware
    const user = await UserModel.findById(userId).select("login_history");
