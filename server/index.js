...
app.use(
  cors({
    origin: (origin, callback) => {
      ...
      // Development: allow localhost and LAN IPs (mobile testing)
      if (!isProduction) {
        const devPattern = /^http:\/\/(localhost|\d{1,3}(\.\d{1,3}){3}):\d+$/;
...
connectDB().then(() => {
  app.listen(PORT, () => {
    console.log(`🚀 Server running on port ${PORT} | ENV: ${isProduction ? "production" : "development"}`);
  });
});
